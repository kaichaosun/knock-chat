...
| Knocking | Address entry with live cost lookup, or from inside a closed chat with the address fixed. Same sheet either way. A pasted invite link is read for the address inside it rather than regrouped into nonsense. |
| Codes | One reader (`readCode`) decides whether a code means a room or a person, so a scan, a paste and a link the app was opened with all arrive at the same two doors. Your own code lives behind the QR button in your profile, with your identicon in the middle of it and error correction raised to Q to pay for what the identicon covers. Scanning uses `BarcodeDetector` where it exists and jsQR where it does not — which on iOS is always — lazily loaded, so the reader costs nothing until the camera is opened. The camera is held only while the sheet is open. |
| Closed chats | Banner with an explicit priced Knock button; composer disabled so no dead message is left behind. |
| Profile | Address with copy and a QR button, your display name, and your own postage price. |
...
| Getting into a group | A link (`?group=<id>`), a pasted link or bare id, a QR code shown in the group's info, or that code scanned in the app. Pasting is matched rather than parsed, so a link that picked up a fragment, a redirect wrapper or trailing punctuation still resolves — and a bare id works, which is what a code read out loud gives you. Nimiq Pay does carry a query string through its deeplink (tested on iOS against `knockchat.org`), so a link opened on a phone arrives with the room still attached. QR costs ~8.5 kB gzip and inherits its colour from CSS, so it stays readable in dark mode. |
...
| What's new | A changelog that ships with the build, since a Mini App has no store listing or release notes to send anybody to. Entries are translation keys held in `lib/changelog`, so a release note reads in whatever language the app is speaking; a test walks every key through both dictionaries, because i18next answers a missing one with the key itself and that has already shipped once. `prefs.seen` holds the newest release a device has been shown — compared by sort order, not equality, so an older build over a newer one does not re-announce what was read. An announcement has to arrive rather than be found, so an unread release is a card above the inbox, the same shape as `KnockRequests` and `GroupRequests` and gone once read or dismissed. The settings row is the archive and carries no mark. |
